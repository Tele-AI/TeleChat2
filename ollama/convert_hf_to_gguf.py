...
class Model:
    ...
    def __init__(self, dir_model: Path, ftype: gguf.LlamaFileType, fname_out: Path, is_big_endian: bool = False,
        ...
        if len(self.part_names) == 0:
            self.part_names = Model.get_model_part_names(self.dir_model, "pytorch_model", ".safetensors")
...
@Model.register("TeleChat2ForCausalLM", "TeleChatForCausalLM", "Telechat2ForCausalLM")
class TelechatModel(Model):
    model_arch = gguf.MODEL_ARCH.TELECHAT2

    @staticmethod
    def _get_or_raise_error_if_not_exist(config, name):
        """
        对于telechat的config.json中的某些字段名称可能会变化，如果找不到特定字段时，会引发异常
        :param config:
        :param name:
        :return:
        """
        try:
            ...
            raise ValueError(f"{name} not in config.json,please modify the file. Config={config}")
        if val is None:
            raise ValueError(f"{name} not in config.json,please modify the file. Config={config}")
        return val
    
    @staticmethod
    def convert_kv_to_normal(kv, n_head):
        assert kv.dim() <= 2
        head_dim = kv.shape[0] // (2 * n_head)
        if kv.dim() == 2:
            hidden_dim = kv.shape[-1]
            kv = kv.view(-1, 2 * head_dim, hidden_dim)
            output_shape = [-1, hidden_dim]
        else:
            kv = kv.view(-1, 2 * head_dim)
            output_shape = [-1]
        k, v = torch.split(kv, head_dim, 1)
        return k.reshape(*output_shape), v.reshape(*output_shape)

    def set_vocab(self):
        self._set_vocab_sentencepiece()
        logger.info("Using SentencePiece vocab")
    ...
    def set_gguf_parameters(self):
        logger.info("set_gguf_parameters")    
        super().set_gguf_parameters()
        
        n_head = self.hparams["n_head"]
        inter_size = self.hparams["ffn_hidden_size"]
        
        ctx_length = 0
        if "base_seqlen" in self.hparams:
            ctx_length = self.hparams["base_seqlen"]
        elif "training_seqlen" in self.hparams:
            ctx_length = self.hparams["training_seqlen"]
        else:
            raise ValueError("gguf: can not find ctx length parameter.")

        rms_norm_eps = self.hparams.get("rms_norm_eps", 1e-05)
        ...
        try:
            ...
            print(e)
        self.gguf_writer.add_rope_freq_base(rope_theta)
        self.gguf_writer.add_feed_forward_length(inter_size)
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if 'self_attention.key_value' in name:
            tensors: list[tuple[str, Tensor]] = []
            # convert k v
            k_weight, v_weight = TelechatModel.convert_kv_to_normal(data_torch, n_kv_head)
            k_weight = LlamaModel.permute(k_weight, n_head, n_kv_head)

            tensors.append((self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_K, bid), k_weight))
            tensors.append((self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_V, bid), v_weight))
            return tensors

        if name.endswith(("self_attention.query.weight", "self_attention.query.bias")):
            data_torch = LlamaModel.permute(data_torch, n_head, n_head)
            logger.info(f"q_proj.weight {name}")
