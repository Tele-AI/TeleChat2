...
def layer_name_mapping(key):
    ...
    layer_rename_map = {
        "model.tok_embeddings.embedding_weight": "transformer.word_embeddings.weight",
        ...
        "model.norm_out.weight": "transformer.ln_f.weight",
    ...
    return f"transformer.{prefix}h.{layer_number}." + layer_rename_map[text]
...
def ms_to_torch(ms_weights):
    ...
    for k, v in ms_weights.items():
        ...
        new_tensor = torch.from_numpy(v.asnumpy())
        torch_params[new_name] = new_tensor
    return torch_params
...
def process_shard_files(config):
    ...
    save_file = config.torch_path + '/' + file_name + '.bin'
